...
public class Tower {
    ...
    public void popLid() {
        for (int i = cups.size() - 1; i >= 0; i--) {
            ...
            if (cups.get(i).hasLid()) {
                ...
                boolean isCovering = (findCup(currentLid.getCompanionSize()) != null);
    ...
    public void swap(String[] o1, String[] o2) {
        ...
        if (c1.isFixed() || c2.isFixed()) {
            fail("No se puede intercambiar una taza fija");
            return;
        }
    ...
    public void cover() {
        ...
        for (int i = 0; i < cups.size(); i++) {
            if (!cups.get(i).isCup()) continue;
            if (i > 0 && !cups.get(i - 1).isCup() && ((Lid) cups.get(i - 1)).isCrazy()) {
                fail("No es posible cubrir: hay una CrazyLid debajo de la taza " + cups.get(i).size());
                return;
            }
            if (!coverIfPossible((Cup) cups.get(i), true)) {
    ...
    public String[][] stackingItems() {
        ...
        for (Stackable c : cups) {
            if (!c.isCup()) continue;
            total += traverseCups((Cup) c, null, null, 0);
        }
        String[][] data = new String[total][2];
        int index = 0;
        for (Stackable c : cups) {
            if (!c.isCup()) continue;
            index = traverseCups((Cup) c, data, null, index);
        }
        return data;
    }
    /**
    ...
    public String[][] swapToReduce() {
        for (int i = 0; i < cups.size() - 1; i++) {
            if (cups.get(i).isFixed()) continue;
            for (int j = i + 1; j < cups.size(); j++) {
                if (cups.get(j).isFixed()) continue;
