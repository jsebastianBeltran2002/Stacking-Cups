...
public class Tower {
    ...
    public void pushLid() {
        ...
        for (int i = cups.size() - 1; i >= 0; i--) {
            ...
            if (!cup.hasLid() && canCover((Cup) cup)) {
                if (totalHeight() + 1 > maxHeight) { showMessage("Excede altura máxima"); lastOperationOk = false; return; }
                cup.addLid();
                reposition();
                lastOperationOk = true;
                return;
    ...
    public void removeLid(int size) {
        ...
        if (c != null) {
            if (c.hasLid()) { c.removeLid(); reposition(); lastOperationOk = true; }
            else { showMessage("Esa taza no tiene tapa"); lastOperationOk = false; }
        } else { showMessage("No existe taza de tamaño " + size); lastOperationOk = false; }
    }
    /**
     ...
     */
    public int height() { return totalHeight(); }
    /**
     ...
     */
    public void exit() { makeInvisible(); cups.clear(); }
    /**
    ...
    private Stackable findCupInner(Cup cup, int size) {
        for (Cup inner : cup.getInnerCups()) {
            if (inner.size() == size) return inner;
    ...
    private boolean canCover(Cup cup) {
        ...
        for (Cup inner : cup.getInnerCups()) {
            ...
            if (!canCover(inner)) return false;
        }
        if (!cup.getInnerCups().isEmpty()) {
            int space = cup.height() - 1;
            if (innerTotal + cup.getInnerCups().size() > space) return false;
        }
        return true;
    }
    ...
    private ArrayList<Cup> findParentListIn(Cup current, Cup target) {
        for (Cup inner : current.getInnerCups()) {
            if (inner == target) return current.getInnerCups();
